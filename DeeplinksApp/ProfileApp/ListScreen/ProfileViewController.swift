...
class ProfileViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.addBarButtons()
        if let redirect = DeeplinkHandler.redirectUrl {
            if redirect == "NewProfile" {
                self.addNewProfile()
            } else {
                let profile = self.profileList.first { (profile) -> Bool in
                    
                    return profile.name?.lowercased() == redirect
                }
            ...
            DeeplinkHandler.redirectUrl = nil
        }
    }
